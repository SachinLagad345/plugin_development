...
// printing book information for shortcode
function book_info_table($args_for_query) {
     $query = new WP_Query($args_for_query);
     $html = '';
    $html .= '<h1> Books Available </h1>
    <table>
        <thead>
            <tr>
                <th> ID </th>
     ...
     if($query->have_posts()) {
        while($query->have_posts())
        {
            ...
            $auth_name = get_metadata('bookinfo',get_the_id(),'author_name_meta',true);
            $price_val = get_metadata('bookinfo',get_the_id(),'price_meta',true);
            $publisher_val = get_metadata('bookinfo',get_the_id(),'publisher_meta',true);
            $year_val = get_metadata('bookinfo',get_the_id(),'year_meta',true);
            $edition_val = get_metadata('bookinfo',get_the_id(),'edition_meta',true);
            $url_val = get_metadata('bookinfo',get_the_id(),'url_meta',true);

           $html .= '<tr>
                <td>' .get_the_id() .'</td>';
            $html .= '<td><a href="' .get_post_permalink() .'">' .get_the_title() .'</a> </td>';
            $html .='<td>' .$auth_name .'</td>';
            $html .= '<td>' .$url_val .'</td>';
            $html .='<td>' .$edition_val .'</td>';
            $html .= '<td>' .$year_val .'</td>';
            $html .='<td>' .$price_val .'</td>';
            $html .='<td>' .$publisher_val .'</td>';
            $html .= '</tr>';
